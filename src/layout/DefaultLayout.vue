          ...
          >
            ...
            <div class="account-grid">
              <div
                ...
                :key="token.id"
                class="account-item"
                @click="handleAccountSelect(token.id)"
              >
                {{ token.name }}
              </div>
            </div>
...
// 检查是否有多个账号
const hasMultipleTokens = computed(() => {
  return tokenStore.gameTokens.length > 1;
});

// 处理账号选择
const handleAccountSelect = async (tokenId) => {
  const selectedToken = tokenStore.selectToken(tokenId);
  if (selectedToken) {
    message.success(`已切换到账号: ${selectedToken.name}`);
  }
};
