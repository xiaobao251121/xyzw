...
const generateQRCode = async () => {
  try {
    ...
    if (!success) {
      throw new Error("二维码获取失败，请检查网络连接或稍后重试");
    }
  } catch (error) {
    const errorMsg = error.message || "未知错误";
    const displayMessage = errorMsg.startsWith("二维码获取失败") 
      ? errorMsg 
      : `二维码获取失败：${errorMsg}`;
    updateStatus(displayMessage, "error");
    console.error("[二维码获取] 生成二维码失败:", error);
 ...
 * 尝试获取微信二维码
 */
const tryGetWeixinQR = async (retryCount = 0) => {
  const maxRetries = 2;
  ...
  try {
    ...
    console.log(`[二维码获取] 正在请求: ${qrPageUrl} (重试次数: ${retryCount})`);
    ...
    if (response.status !== 200) {
      ...
      let errorMessage = `HTTP 状态码：${statusCode}`;
    ...
    if (html.includes("error") || html.includes("Error") || html.includes("ERROR")) {
      ...
      if (errorMatch) {
        throw new Error("服务返回错误：" + errorMatch[1]);
      }
      throw new Error("服务返回异常响应");
    }
    ...
    if (!qrUrl) {
      ...
      for (const img of imgElements) {
        const src = img.src;
        if (src && (src.includes("qrcode") || src.includes("QRCode") || src.includes("qr"))) {
          qrUrl = src;
          break;
        }
      }
    ...
    if (!qrUrl) {
      throw new Error("未找到二维码图片地址，请检查网络代理配置");
    }
    ...
    // 开始轮询扫码状态
    startScanMonitoring();
    console.log("[二维码获取] 成功获取二维码: " + qrUrl);
    return true;
  } catch (err) {
    console.error("[二维码获取] 失败:", err);
    
    // 重试逻辑
    if (retryCount < maxRetries) {
      console.log(`[二维码获取] 第 ${retryCount + 1} 次失败，准备重试...`);
      updateStatus(`二维码获取失败，正在重试 (${retryCount + 1}/${maxRetries})...`, "info");
      
      await new Promise(resolve => setTimeout(resolve, retryDelay));
      return tryGetWeixinQR(retryCount + 1);
    }
    
    // 最终失败
    const errorMessage = `二维码获取失败：${err.message}`;
