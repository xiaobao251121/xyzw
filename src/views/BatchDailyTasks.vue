  ...
  <div class="batch-daily-tasks">
    ...
    <div class="page-header-wrapper">
      ...
      <div class="header-top">
        <div class="title-row">
          <h2 class="page-title">批量日常任务</h2>
          <div class="scheduled-info">
            <span class="info-item">共 {{ scheduledTasks.length }} 个定时任务</span>
            <span v-if="shortestCountdownTask" class="info-item highlight">
              即将执行：{{ shortestCountdownTask.task.name }} ({{ shortestCountdownTask.countdown.formatted }})
            </span>
            <span v-else class="info-item">暂无定时任务</span>
          </div>
        </div>
      </div>
    ...
    <div class="main-content">
      ...
      <div class="content-left">
        <!-- Token Selection -->
        <n-card title="帐号列表" class="token-list-card">
            ...
            <n-space vertical style="width: 100%">
              ...
              >
                <div
                  ...
                  "
                >
                  <label class="section-label">分组选择</label>
                  <n-button
                    size="small"
                    type="error"
                    text
  ...
  if (autoScrollLog.value && logContainer.value) {
    nextTick(() => {
      try {
        ...
        console.warn("Failed to scroll log container on change:", error);
      }
    });
  }
}, { deep: true });
...
.settings-content :deep(.n-divider__line) {
  border-color: var(--divider-color);
}

/* ===== 活动日历样式 ===== */
.page-header-wrapper .activity-section {
  flex: 1;
  margin-right: 16px;
  padding-right: 16px;
